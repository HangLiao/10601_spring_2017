...
def pooling_layer_forward(input, layer):
  ...
  for n in range(batch_size):
    ...
    assert col.shape == (k * k * c, h_out * w_out), 'col has incorrect shape!'

    for i in range(0, k*k*c, k*k):

      for j in range(h_out*w_out):
        max = col[i][j]

        for x in range(i, i+k*k):
          if (col[x][j] > max):
            max = col[x][j]

        output['data'][c*j + i/(k*k)][n] = max
...
def pooling_layer_backward(output, input, layer):
  ...
  for i in range(batch_size):
    ...
    col = im2col_conv(input_n, layer, h_out, w_out)
    x = col.reshape(k*k, h_out*w_out*c)
    y = np.zeros(x.shape)
    for j in range(h_out*w_out*c):
      ind = np.argmax(x[:,j])
      y[ind][j] = 1.0
    t = y * output['diff'][:,i]
    s = np.reshape(t, col.shape)
    m = col2im_conv(s, input, layer, h_out, w_out)
    input_od[:, i] = m.flatten()
  ...
  for i in range(output['batch_size']):
    for j in range(l):
      ...
      if (tmp <= 0):
          ...
          input_od[j][i] = 1 * output['diff'][j][i]
...
def inner_product_forward(input, layer, param):
  ...
  for i in range(batch_size): #in_width??
    col = x[:,i]
    output['data'][:,i] = np.dot(w.T,col) + b
...
def inner_product_backward(output, input, layer, param):
  ...
  (m,n) = input['data'].shape
  (x,y) = output['diff'].shape
  ...
  for i in range(batch_size):
    ...
    a = np.reshape(input['data'][:,i], (m,1))
    b = np.reshape(output['diff'][:,i], (1,x))
    param_grad['w'] += np.dot(a,b)
    input_od[:,i] = param['w'].dot(output['diff'][:,i])
